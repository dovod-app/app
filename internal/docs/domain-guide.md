...
**A flag is not only a route.** Most of them gate a route outright, but the cross-reference routes are mounted ungated — documents are always in a link, so the route always answers — and the rows they serve are filtered one at a time instead. A reference is written by a task or by an answer as readily as by a document, and each row carries the source's id and the code it cites; a link with `tasks` off therefore serves the document-sourced rows and withholds the rest, and the `summary` counts what it served. Resolution is filtered the same way: a resolved `[[T4]]` says *this research really has a T4*, so under a link with `tasks` off the row comes back inert rather than confirming it. The rule generalises — **a route that serves an optional part must gate itself** — and the other payloads that carry one are the research route's `active_session`, the export's counts, roadmap node `ref_data`, and the Obsidian vault.
